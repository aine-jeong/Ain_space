 ...
 * for(제어 변수 선언과 초기화; 반복 조건식; 제어변수의 변화식) {
 * 		반복할 코드
 * }
 * 
 */
...
//		      
//		   2. 제어 변수의 초기화나 조건식에는 다른 변수가 사용 가능하다.
...
//		   3. 비록 우리가 "전통적으로" 제어변수는 i -> j -> k -> l 이런식으로 들어가지만
//		           여러분들이 불편하다면 굳이 제어변수의 이름을 i라고 안해줘도 된다.
//		    
//		   4. 조건식과 변화식은 주로 뭐보다 작을동안, ++ 을 쓰지만
//		   	  true/false가 나오는 조건식과 제어변수의 값을 변화시킬 수 있는 할당연산자면 뭐든 사용 가능하다.
		for(int i = 100000; i > 0; i -= 100000) {
			System.out.println("i의 현재값: +i");
			
			
//			다른 제어문들도 해당되는 내용이지만 () 뒤에 ;이 붙으면 안된다.
			for(int j = 1; j <= 1000; j++) { 
				//for(int i = 1; i <= 1000; i++); 처럼
				// () 뒤에 ; 붙인 경우
				// 반복할 코드가 없다는 내용이 된다.
				System.out.println("몇 번 반복될까?"); // 1번 반복됨..!!! if나 while문도 마찬가지다.
			}
